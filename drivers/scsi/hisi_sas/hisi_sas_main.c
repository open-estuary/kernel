...
static int
hisi_sas_debug_issue_ssp_tmf(struct domain_device *device, u8 *lun,
...
static void hisi_sas_bytes_dmaed(struct hisi_hba *hisi_hba, int phy_no)
{
	...
	if (sas_phy->phy) {
		...
		sphy->minimum_linkrate_hw = phy->minimum_linkrate;
			     ...
			     int abort_flag, int tag)
{
	...
	if (!(hisi_hba->hw->flags & HISI_SAS_SUPP_INTERNAL_ABT))
		return -EOPNOTSUPP;
...
static void hisi_sas_dev_gone(struct domain_device *device)
{
	...
	if (sas_dev) {
		...
		dev_info(dev, "found dev[%lld:%x] has gone\n",
			 sas_dev->device_id, sas_dev->dev_type);
	}
					   ...
					   struct hisi_sas_tmf_task *tmf)
{
	...
	for (retry = 0; retry < TASK_RETRY; retry++) {
		...
		if (task->task_status.resp == SAS_TASK_COMPLETE && //0
			task->task_status.stat == SAS_DATA_OVERRUN) { //0x83
