...
{
	unsigned short physicalID,vendorID;
	char physicalRev;
	logical_chip_type_t chip;
	
	vendorID = vendorId750; /* either 126f or 19e5 */
	physicalID = devId750;//either 0x718 or 0x750
	physicalRev = revId750;

	if(physicalID == 0){
		physicalID = FIELD_GET(PEEK32(DEVICE_ID),DEVICE_ID,DEVICE_ID);
		physicalRev = FIELD_GET(PEEK32(DEVICE_ID),DEVICE_ID,REVISION_ID);
	}
	...
	else if(vendorID == 0x19e5)
	{
		/*
		 * SM750HS_F and SM750HS_A uses same ID,but different revision number
		 * Treat them same chip during validation
		 * */
		chip = SM750HS_F;
	}
    else
...
//1. outer_bypass_n=0
	 tmp0 = PEEK32(CRT_PLL1_750HS);
	 tmp0 &= 0xBFFFFFFF;
        POKE32(CRT_PLL1_750HS, tmp0);

//2. pll_pd=1?inter_bypass=1
        POKE32(CRT_PLL1_750HS, 0x21000000);

// 3. config pll 
        POKE32(CRT_PLL1_750HS, pll);

// 4. delay 5 us
	 delay(1);       /* delay for 1 milisecond */
	//waitMasterClock(1); // 1 milisecond
	
// 5. pll_pd =0
	 tmp1 = pll & ~0x01000000;
        POKE32(CRT_PLL1_750HS, tmp1);
...
unsigned int ddk750_getVMSize()
{
	...
	if(getChipType() >= SM750LE)
		return MB(16);/* sp605 has 64mb,but on chip huawei's 750 got only 16mb */
...
int ddk750_initHw(initchip_param_t * pInitParam)
{
	...
	if(getChipType() < SM750LE){ /* 718 or 750*/
